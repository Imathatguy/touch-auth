...
from defend_touch import generate_protection_noise, split_and_combine
...
def main_run(selection, ratio):
    ...
    if resample_users:
        ...
        random_data = pd.read_csv(random_loc, header=0, index_col=0)
        temp_a = pd.get_dummies(random_data['udlr_flag'])
        random_data[['1_f', '2_f', '3_f', '4_f']] = temp_a
        random_data.drop('udlr_flag', axis=1, inplace=True)
        random_data = random_data.astype(float)

        random_vdata = a.normalize_external(random_data.values)
    ...
    path = os.path.join(tmp_path, "models", "touch")
    mkdir_p(path)
